...
#include <QFormLayout>
#include <QDebug>
User::User(int userID , QString name ,QString lastname ,QString password , Role level , QWidget* parent)
    :QWidget(parent) ,userID(userID) ,  name(name) , lastname(lastname),password(password),userLevel(level){
    setupUI();
}
...
void User::updateProfile()
{
    ...
    bool User::login(const QString& name , const QString& lastname , const QString& password)
    {
